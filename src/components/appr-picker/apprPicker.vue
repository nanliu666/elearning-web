...
import { submitApprApply, getUserOrgList, getUserBizList } from '@/api/apprProcess/apprProcess'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      noMatchOrg: null,
      rules: {
        orgId: [{ required: true, trigger: 'change', message: '请选择所在部门' }],
        // 设置业务部门为非必填
        // bizId: [{ required: true, trigger: 'change', message: '请选择所在业务部门' }],
  ...
  computed: {
    ...
    // 流程出错
    processErrored() {
      return this.conditionFieldsFullfilled && !this.conditionFullfiled
    },
    // 流程条件是否完善
    processFullfilled() {
      // return !!this.orgId && !!this.bizId && this.conditionFieldsFullfilled
      return !!this.orgId && this.conditionFieldsFullfilled
    }
  },
  watch: {
    orgId() {
  ...
  methods: {
    ...
    getUserBizList() {
      getUserBizList({ userId: this.userId }).then((res) => {
        this.userBizList = res
        if (res.length === 1) {
    ...
    checkApprovers() {
      ...
      const loop = ($el) => {
        let data = $el.data
        // 设置data.noData属性来实现跳过审批人功能。
        if (!data || (!data.noData && data.type === 'approver' && data.userList.length === 0)) {
          valid = false
        }
      ...
      const loop = ($el) => {
        ...
        if (data.variable) {
          if (
            ...
            ['multiDepartmentHeads', 'multiCompetentBusiness'].includes(
              data.properties.assigneeType
            )
          ) {
            map[data.variable] = []
          } else if (
            ...
            // 处理指定层级的业务主管与部门领导也兼容多级
            const { userList } = data
            if ($el.isMultiApprover) {
              _.each(userList, (user) => {
                map[data.variable].push(`taskUser_${user.userId}`)
              })
            } else {
              map[data.variable] = _.map(userList, (user) => `taskUser_${user.userId}`).join()
            }
          } else {
