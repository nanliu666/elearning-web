...
export default ({ mock }) => {
  ...
  // 发起人组织架构主管查询接口
  Mock.mock(new RegExp('/appr/v2/appr/user/org/leader' + '(\\?.*)?$'), 'get', (req) => {
    // 先排除查询为空数组的情况
  ...
  // 发起人业务架构主管查询接口
  Mock.mock(new RegExp('/appr/v2/appr/user/biz/leader' + '(\\?.*)?$'), 'get', (req) => {
    const response = _.times(_.random(10), () =>
      Mock.mock({
        userId: '@id()',
        name: '@cname()',
  ...
  Mock.mock(new RegExp('/appr/v2/appr/user/role' + '(\\?.*)?$'), 'get', (req) => {
    const response = _.times(_.random(1, 10), () =>
      Mock.mock({
        userId: '@id()',
        name: '@cname()',
        workNo: _.random(100),
  ...
  // 按分类查询角色接口
  Mock.mock(new RegExp('/sys/v1/role/category/role' + '(\\?.*)?$'), 'get', (req) => {
    const response = _.times(_.random(1, 10), () =>
      Mock.mock({
        id: '@id()',
        name: '@cname()',
        roles: _.times(_.random(10), () =>
          Mock.mock({
            roleId: '@id()',
            roleName: '@cname()'
          })
