 ...
 * 发起人指定角色查询接口
 * @param {object} params 参数
 * @param {string} [params.roleId] 角色ID，多个以逗号分隔
 * @returns {Array<{userId: string; name: string; workNo: string; avatarUrl: string}>}
 */
export const getUserRole = (params) => get('/appr/v2/appr/user/role', params)

/**
 * 按分类查询角色接口
 * @param {object} params 参数
 * @param {string} [params.roleName] 角色名称，用于模糊搜索
 * @returns {Array<{id: string; name: string; role: Array<{roleId: string;  roleName: string}>}>}
 */
export const getCategoryRole = (params) => get('/sys/v1/role/category/role', params)
