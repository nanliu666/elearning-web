<template>
  <div>
    <div class="catalog-header">
      <div
        class="catalog-header-item"
        @click="$router.push('/training/catalog')"
      >
        培训项目分类
      </div>
      <div class="catalog-header-item active">
        线上必修安排分类
      </div>
    </div>
    <basic-container
      block
      class="basic-container1"
    >
      <common-table
        id="demo"
        ref="table"
...
<script>
import { getCategoryTree, updateCategoryStatus, deleteCategory } from '@/api/learnPlan/index'
import { getCreatorList2 } from '@/api/live'
        ...
        requireOptions: [
          {
            ...
            data: '',
            options: [],
            config: { placeholder: '输入分类名称搜索', 'suffix-icon': 'el-icon-search' }
          }
        ],
        popoverOptions: [
          {
...
.catalog-header {
  ...
  .catalog-header-item {
    ...
    color: #333;
    &.active {
      border-bottom: 4px solid #01aafc;
    }
  }
}
