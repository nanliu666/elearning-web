...
export default {
  ...
  computed: {
    ...
    personList: {
      get: function() {
        return this.model.participantsList
      },
      set: function(value) {
    ...
    orgList: {
      get: function() {
        if (this.planId) {
          _.forEach(this.model.orgIds, (x) => {
            x['$requestApiName'] = 'getOrgChild'
          })
        }
        return this.model.orgIds
      },
      set: function(value) {
        value = value.filter(
          (item) => !value.find((v) => item.parentIds && item.parentIds.includes(v.id))
        )
        this.model.orgIds = value
      }
    },
    positionList: {
      get: function() {
        if (this.planId) {
          _.forEach(this.model.positionIds, (x) => {
            x['$requestApiName'] = 'getPostionUserChild2'
          })
        }
        return this.model.positionIds
      },
      set: function(value) {
        this.model.positionIds = value
      }
    },
    curOrgList: {
      get() {
  ...
  methods: {
    getOrgAmount(target) {
      ...
      return list.reduce((total, next) => (total += next.workNum || 0), 0)
    },
