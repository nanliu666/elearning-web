  ...
  <div>
    <div :class="['newApprove', isParallel ? 'is-parallel' : '']">
      ...
      >
        ...
        <!-- <el-step
          v-for="(item, index) in waitProgress"
          :key="index + 'i'"
        >
          <template
            slot="icon"
          ...
          <template slot="title">
            <div class="title">
              <div>
                ...
                <span style="margin:0 12px;display: inline-block">{{ item.approveTime }}</span>
                <span
                  v-if="item.nodeId !== 'start' && item.hasOwnProperty('result')"
                  :class="[item.result, !item.result ? 'result' : '']"
                >
                  {{ item | result }}
                </span>
                <span
                  v-else-if="item.hasOwnProperty('result')"
                  class="start"
                >
                  <span v-if="item.result === 'Cancel'" /> <span v-else>发起</span>
                </span>
              </div>
              <div>
                <span
