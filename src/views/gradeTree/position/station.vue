...
export default {
  ...
  methods: {
    handlerDeleteAll(list) {
      this.$confirm('您确定要删除你选中的职务吗?', {
        ...
        if (name) {
          ...
          this.$confirm(`很抱歉，您选中的职务 ${name} 下存在员工，请先将员工调整后再删除`, {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
    ...
    handleDelete(row) {
      if (row.workNum) {
        this.$confirm('很抱歉，您选中的职务下存在员工，请先将员工调整后再删除', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
