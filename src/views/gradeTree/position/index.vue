<template>
  <div>
    ...
    <nav
      ...
      class="nav"
    >
      <span>使用指南：岗位是指企业的某个员工需要完成的具体任务，是员工最基础的工作属性，如：司机、行政专员等</span>
      <span
        class="el-icon-close"
          ...
          <el-main>
            <div style="margin-top: 20px">
              ...
              >
                ...
                >
                  <div>
                    {{ scope.row.jobSkills.length > 0 ? scope.row.jobSkills.join('、') : '' }}
                  </div>
                </template>
...
export default {
  ...
  methods: {
    ...
    handlerDeleteAll(list) {
      ...
      this.$confirm('您确定要删除你所选中的岗位吗?', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        let params = {
          ids: list.map((i) => i.jobId).join(',')
        }
    ...
    searchJob: _.debounce(function() {
      this.getCategory(this.form.jobName)
    }, 500),
