...
export default {
  ...
  methods: {
    ...
    handleUpdate(item) {
      if (item) {
        let { id, name, parentId, description } = item
        if (parentId === '0') {
          parentId = ''
        }
        Object.assign(this.form, { id, name, parentId, description })
    ...
    forEach(list) {
      return list.map((item) => {
        ...
        return {
          id,
          name,
          children: this.forEach(children),
          description,
          updateTime,
          creatorName,
          parentId
    ...
    addProperty(list, property, condition) {
      list.forEach((item) => {
        let res = condition(item)
        if (res) {
          Object.assign(item, property)
          this.addProperty(res, property, condition)
        }
      })
    }
  }
