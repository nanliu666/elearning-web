...
const rowTemplates = {
  detail(h, element) {
    ...
    const addPart = (
      ...
      >
        ＋ {element.__config__.actionText || '添加'}
      </div>
    )
    const wrapItem = () => {
      return (
...
const layouts = {
  ...
  colFormItem(h, scheme) {
    ...
    let renderItem = ''
    // 检测每个label的权限输出，请不要删除
    // console.log('scheme.__config__.label', scheme.__config__.label, formPrivilege)
    ...
    switch (formPrivilege) {
      case 1:
        ...
        if (this.formConfCopy.isDetail) {
          // 详情页，有默认值显示默认值，无默认值不显示这个标签
          let defaultValue = scheme.__config__.defaultValue
          if (scheme.__config__.type === 'desc') {
            renderItem = ''
          } else if (scheme.__config__.type === 'relevant') {
            // 关联审批
            scheme.__pc__.props.hasOperate = false
            renderItem = !_.isEmpty(scheme.__config__.defaultValue) ? wrapItem(true) : ''
          } else if (scheme.__config__.type === 'locationPicker') {
            renderItem = defaultValue.location !== '' ? wrapItem(false) : ''
          } else {
            scheme.__mobile__.props.disabled = true
            let flag = !_.isEmpty(defaultValue) || _.isNumber(defaultValue)
            renderItem = flag ? wrapItem(false) : ''
          }
        } else {
          // 审批发起页面，只读权限表现为置灰处理
...
export default {
  ...
  methods: {
    ...
    genFormFields(form) {
      const formFields = []
      _.each(this.formConfCopy.fields, (item) => {
