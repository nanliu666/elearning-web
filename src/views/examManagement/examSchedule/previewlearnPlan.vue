...
export default {
  ...
  methods: {
    ...
    deletePreview() {
      delExamPreview({ id: this.paperId }).then(() => {
    ...
    initData() {
      this.loading = true
      let loadFun = this.paperType === 'manual' ? getManualPreview : getRandomPreview
      if (this.isManaged && this.paperType === 'random') {
