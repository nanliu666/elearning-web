        ...
        >
          <template #categoryIds-label-slot>
            <span>
              ...
              <span>
                <span>（题库试题总数：</span>
                <span>{{ formData.totalQuestionNum }}</span>
                <span> ）</span>
              </span>
            </span>
          </template>
          <template #questionNum-label-slot>
            ...
            <el-tooltip
              class="item"
              effect="dark"
              content="考试时从题库中随机抽取出现的试题数，且不能大于题库试题总数"
              placement="top-start"
            >
...
<script>
import { QUESTION_TATOL_ARR, QUESTION_TYPE_SINGLE } from '@/const/examMange'
...
export default {
  ...
  methods: {
    ...
    calculateTotalNum(val) {
      ...
      this.formData.totalQuestionNum = _.set(_.sum(relatedNumList), 0)
    ...
    validateTotalQuestionNum(rule, value, callback) {
      ...
      if (value > this.formData.totalQuestionNum) {
