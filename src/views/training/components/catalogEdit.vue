...
import { getCategoryTree, addCatalog, editCategory } from '@/api/train/train'
export default {
  name: 'CatalogEdit',
  props: {
    visible: {
  ...
  methods: {
    ...
    async loadOrgTree() {
      let res = await getCategoryTree({ addFlag: '1' })
      if (this.type === 'edit') {
        this.orgTree = this.clearCurrentChildren(res)
        this.parentOrgIdLabel = this.form.parentId?this.findOrg(this.form.parentId).name : undefined
      ...
      this.$refs.ruleForm.validate((valid, obj) => {
        ...
        if (valid) {
          if (this.type !== 'edit') {
            ...
            this.loading = true
            editCategory(_.assign(this.form, { addFlag: '1' }))
              .then(() => {
